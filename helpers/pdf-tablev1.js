...
async function checkCellHeight(width, height, properties) {
    ...
    if(typeof properties != 'undefined' && typeof properties.text != 'undefined') {
        ...
        var textWidth = (typeof properties != 'undefined' && typeof properties.text != 'undefined' && typeof properties.text.text != 'undefined' && properties.text.text != null && typeof properties.text.font != 'undefined') ? 
            properties.text.font.widthOfTextAtSize(currentText, properties.text.fontSize) : width ;
        var textLength = 0;
...
async function createCell(pdfDoc, currentPage, x, y, width, height, properties, cell_height) {
    ...
    if(typeof properties != 'undefined' && typeof properties.color != 'undefined') {
        //currentProps['color'] = properties.color;
        var path2 = "M " + x + " " + y 
                    + " l " + width + " " + 0 
                    + " l " + 0 + " " + cell_height 
    ...
    if (properties.borderTop && properties.borderTop == "none") {}
        ...
        path +=  " l " + width + " " + 0;
        console.log("drawing with current props " + width)
        console.log(currentProps)
...
async function insertTextInTheMiddle(pdfDoc, currentPage, x, y, cellwidth, cellheight, text) {
    ...
    if(Array.isArray(text.text)) {
        for(textS of text.text) {
            ...
            const {width, height} = currentPage.getSize();
            x = xDefault + cellwidth / 2 - textWidth / 2;
...
async function insertTextInTheLeft(pdfDoc, currentPage, x, y, cellwidth, cellheight, text) {
    if(Array.isArray(text.text)) {
        ...
        var textWidth = (typeof properties != 'undefined' && typeof properties.text != 'undefined' && typeof properties.text.text != 'undefined' && properties.text.text != null && typeof properties.text.font != 'undefined') ? 
            properties.text.font.widthOfTextAtSize(currentText, properties.text.fontSize) : cellwidth ;
        const textHeight = text.font.heightAtSize(text.fontSize);
        const {width, height} = currentPage.getSize();
        //y = y + text.fontSize - (cellheight /2 - text.fontSize / 2);
        y = y + textHeight + (cellheight/4 - text.fontSize / 3);
...
async function insertTextJustified(pdfDoc, currentPage, x, y, cellwidth, cellheight, text) {
    if(Array.isArray(text.text)) {
        for(textS of text.text) {
            const textWidth = text.font.widthOfTextAtSize(textS, text.fontSize);
            const textHeight = text.font.heightAtSize(text.fontSize);

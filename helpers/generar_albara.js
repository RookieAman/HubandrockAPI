...
const PREFIX = "ALB";
const FOLDER = "albarans";
const title = "ALBARA"

exports.generarAlbara = function(id, callback) {
    modelAlbara.get(id, function(data_factura) {
            ...
            modelConfiguracio.get(function(data_configuracio) {
                modelEmpresa.getDefault(function(dataEmpresa) {
                    ...
                    if(typeof configuracio.plantilla_facturacio !== 'undefined' 
                        ...
                        var codi = (typeof factura.codi !== 'undefined') ? factura.codi.replace("/", "_") : "00";
                        creacioTable(factura, client, empresa, configuracio, factura_url, "files/" + FOLDER + "/" + PREFIX + "-" + codi + ".pdf", function(data_res) {
                            callback(data_res);
                        })
                    }
                });
...
exports.generarAlbaraBlank = function(id, callback) {
    modelAlbara.get(id, function(data_factura) {
        var factura = data_factura.row
            ...
            modelConfiguracio.get(function(data_configuracio) {
                modelEmpresa.getDefault(function(dataEmpresa) {
                    ...
                    if(typeof configuracio.plantilla_facturacio !== 'undefined' 
                        ...
                        && configuracio.plantilla_facturacio == constants.FACTURA_TAULA) {
                        var codi = (typeof factura.codi !== 'undefined') ? factura.codi.replace("/", "_") : "00";
                        creacioTable(factura, client, empresa, configuracio, "files/invoice-templates/blank.pdf", "files/" + FOLDER + "/" + PREFIX + "-" + codi + ".pdf", function(data_res) {
                            callback(data_res);
                        })
                    }
                });
...
async function creacio(factura, client, configuracio, url_template, url_save, callback) {
    ...
    currentPage.drawText('Factura ' + factura.codi, {
        x: xAlpha * 4.1,
        y: height + (6.2 * yAlpha + fontSizeSubtitle) * despl,
        size: fontSizeSubtitle,
        font: universBold,
        ...
        if(descompte) {
            rows.push({
                cells: [
                    ...
                    {width: 2.1 * xAlpha, height: 0.6 * yAlpha, paddingTop:  0.05 * yAlpha, textAlign: 'center', borderRight: "none", borderBottom: "none", borderLeft: "none", borderTop: "none", text: {text: linia.descompte_percentatge + "%", font: universLight, fontSize: 9}},
    ...
    callback({code: 1, url_albara: url_save})
}


async function creacioTable(factura, client, empresa, configuracio, url_template, url_save, callback) {
    //pdf creation
    ...
    capcaleraFactura.factura_nom= "Albarà: ";
    capcaleraFactura.factura_nom_x = xAlpha * 3.6;
    currentPage.drawText('Albarà:', {
        x: xAlpha * 2,
